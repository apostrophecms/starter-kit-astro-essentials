...
- **Client-side JavaScript** - A web component has to be registered independently of whether its widget rendered on the page, because `customElements.define()` writes to a single document-global registry. So do not put an inline `<script>` in a widget's `.astro` file: Astro bundles a component-scoped script only for pages that server-render that component, and a widget added in-context by an editor never has its script loaded. Put the component in its own module and register it from `src/widgets/players.ts`, which `[...slug].astro` loads at page level. See `VideoWidget.astro` / `VideoWidget.ts` for the reference implementation.
